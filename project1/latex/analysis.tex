\section{Analysis}\label{sec:analysis}
    ...
    Moving forward, we essentially repeat the analysis in \Sec{reganalysis_franke} for the terrain data. Starting with the OLS method, we expect the need of a higher order polynomial than for the Franke funtion.
        ...
        After performing a simple $k$-fold cross-validation to optimise the model, we see that it clearly prefers order $d=6$. We continue with the bootstrap resampling technique to verify that this complexity is preferable for our test data as well. In \Fig{gc_model_comlexity} we see the expected behavoiur of the training and prediction error per increasing model complexity.
