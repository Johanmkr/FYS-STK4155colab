\section{Analysis}\label{sec:analysis}
    ...
    \subsection{Regression analysis for Franke function}\label{sec:reganalysis_franke}
        ...
        \subsubsection{OLS}\label{sec:olsanalysis}
            ...
            We have scaled the data and removed the intercept, thus we do not concern ourselves with $\beta_0$, so $\svec{\beta}^{\text{OLS}} = (\beta_1, \beta_2, \beta_3, \dots, \beta_{p-1})$. We train models, i.e. we find $\optbeta^{\text{OLS}}$ for polynomials up to order $5$. The values of these optimal parameters are plotted in \Fig{beta_with_standard_deviation}, with error bars of one standard deviation. We notice that the variation of the feature parameters increase as the polynomial degree of the model increase. This is expected because when the complexity of the model increase, it want to traverse more points exactly. We further see that the coefficients of the same features more or less have the same sign for the different polynomial degrees. This is a sign of a stable model, as we do not want these coefficient to change much when we use different polynomial degrees for our model.
    ...
    Moving forward, we essentially repeat the analysis in \Sec{reganalysis_franke} for the terrain data. Starting with the OLS method, we expect the need of a higher order polynomial than for the Franke funtion.
        ...
        After performing a simple $k$-fold cross-validation to optimise the model, we see from \Fig{gc_cv_errors_ols} that it clearly prefers order $d=6$. We continue with the bootstrap resampling technique to verify that this complexity is preferable for our test data as well. In \Fig{gc_model_complexity} we see the expected behavoiur of the training and prediction error per increasing model complexity.
        ...
        The error heatmap in \Fig{gc_gridsearch_ridge} favourises the combination $d=18, \lambda = 1.23\cdot 10^{-4}$. In an attempt to verify this model, we do the bootstrap for this polynomial degree and the same hyper parameters. The resulting train and test MSE is shown in \Fig{gc_mse_ridge_bs}.
        ...
        There are actually model with $d=15\pm 1, \lambda\sim  10^{-8}$ which give cross-validation accuracy $\lesssim 0.16$ for various choices of $k$. However, when confirming these models using bootstrap, the prediciton errors grow huge, as do both the variance and squared bias, in the regime of $\lambda\sim 10^{-8}$. There is a sudden drop in error and variance for $\lambda \simeq 8.5 \cdot 10^{-9}$ for $d=15$, but this now seems random.
        ...
        There is little point in continuing with validation of this model. In this report, we omit the rest of the Lasso analysis due to 
        ...
        \begin{enumerate}[label=(\roman*)]
            ...
            \item a wish to reduce the number of figures for the sake of a legible paper.
        \end{enumerate}
