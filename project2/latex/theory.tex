...
\subsection{Classification}\label{sec:classification}
    ...
    When evaluating the loss we use \Eq{logistic_regression_cost_function}
...
\end{equation}
so that $\hat{y}^{(i)} = p\big(\vec{x}^{(i)}\big)$.
...
\end{equation}
where $\hat{y}$ is the output we get from our model at corresponding $\vec{x}\in\mathcal{D}^\mathrm{test}$. The MSE is then a function of our model, e.g. of the parameter vector $\svec{\theta}$. For the binary classification problem, it makes sense to use the accuracy score
