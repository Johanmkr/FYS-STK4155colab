...
\subsection{Stochastic gradient descent (SGD)}\label{sec:stochastic_gradient_descent}
    ...
    We can mitigate parts of the struggle with hyperparameter adjustment by using an algorithm with adaptive learning rates. We present a few such schemes in short below.

    \begin{enumerate}[leftmargin=0pt,labelwidth=!,labelsep=.05em]
        \item[]\textbf{AdaGrad} (Algorithm 8.4 in \citep{Goodfellow2016}) \comment{Works well with convex functions}
        \item[]\textbf{RMSProp} (Algorithm 8.5 \& 8.6 in \citep{Goodfellow2016}) 
        \item[]\textbf{Adam} (Algorithm 8.7 in \citep{Goodfellow2016})
    \end{enumerate}
...
\subsection{Neural Network (NN)}\label{sec:neural_network}
    ...
    The matrix of weights $W^{l-\! 1\to 1} \in \RR[N_{l-\!1} \cross N_l]$ 
    \fillertext
    
    The activation function $g_l \,:\, \RR[N_l] \to \RR[N_l]$ 
    \fillertext
    ...
    \begin{equation}
        \begin{split}
            ...
            &\quad \vec{a}^l = (W^{l-\! 1\to l})\TT\vec{h}^{l -\!1} + \vec{b}^l \,;
        \end{split}
    \end{equation}

    where \checkthis{$\hat{\vec{y}} = \vec{h}^L$} is the output.
