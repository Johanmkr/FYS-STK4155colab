...
and is indeed the sigmoid function in eq. \eqref{eq:sigmoid} if we substitute $\svec{\xi} \to \beta_0 + \beta_1 \xi$, $\beta_0, \beta_1 \in \RR$. This can be generalised to 


\begin{equation}\label{eq:logistic_regression_cost_function}
    \mathcal{L}(\svec{\theta}) = -  \frac{1}{2n}\sum_{i=1}^n \Big[ y^{(i)} \big()^{(i)}\big) - \log{\Big(1+e^{\theta_0 + \svec{\theta}\TT \vec{x}^{(i)}}\Big)} \Big]
\end{equation}
