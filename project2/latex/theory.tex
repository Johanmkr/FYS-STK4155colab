...
\subsection{Stochastic gradient descent (SGD)}\label{sec:stochastic_gradient_descent}
    ...
    \end{equation}
    $\lambda$ is the penalty term set to zero for OLS and a small positive value for Ridge regression. The output $\hat{y}^{(i)}$ is resulting from some function evaluated at $\vec{x}^{(i)}$, which for our purposes is $\hat{y}^{(i)} = [\vec{x}^{(i)}]\TT \svec{\theta}$.
...
\subsection{Regression}\label{sec:regression}
    When using a FFNN for a regression problem we opt to fit a function to a certain data set, and train the network to give one output (function value) per input. One approach to this is to let the number of features be the dimensionality of the input data, and the number of data points be the data points in the domain we are considering. In this investigation we will use the two dimensional Franke function from \projectOne\footnote{Equation 10} on a $20\cross 20$ grid (more on this later).
    
    Since we want to find a function to data points, the obvious way of measured the error from the output layer is by considering the mean squared error (MSE). Thus, our loss function will as given in \Eq{ridge_cost_func}
