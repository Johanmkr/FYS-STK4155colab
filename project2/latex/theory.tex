...
\subsection{Neural Network (NN)}\label{sec:neural_network}
    ...
    The structure of such a chain-based architecture is described by the $L$ hidden layers $\vec{h}^l,\,l=1,2, \dots, L$, given by
    ...
    where we defined $\vec{h}^0$ to be the input layer. The activation function $g$ \fillertext
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item\label{item:build2} Define the model and desgin its architecture  \fillertext[number of layers etc?]
        \item\label{item:build3} Choose loss function and optimiser \wtf[please send help]
        \item\label{item:solve1} Train the network \rephrase{(find the right weights and biases)}
